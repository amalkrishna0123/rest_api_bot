...
# ============================================================================
# PERFORMANCE OPTIMIZATION: Pre-compiled regex patterns for parse_fields
# ============================================================================
import re

# Compile patterns once at module level to avoid recompilation
EMIRATES_ID_PATTERN = re.compile(r'\b\d{3}-\d{4}-\d{7}-\d\b')
DATE_PATTERN = re.compile(r'(\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4})')
NAME_PATTERNS = [
    re.compile(r'(?i)Name\s*[:\-]?\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)'),
    re.compile(r'(?i)Name\s*[:\-]?\s*([A-Z][^\n\r]{1,60}?)(?:\s+Name\s*$|\n|Occupation|Date|Nationality|Expiry|Issuing|Sex|Gender|$)'),
    re.compile(r'(?i)(?:الاسم|اسم)\s*[:\-]?\s*([^\n\r]{2,80})'),
]
NATIONALITY_PATTERN = re.compile(r'(?i)(?:Nationality|الجنسية)\s*[:\-]?\s*([^\n\r]{2,80})')
GENDER_PATTERNS = [
    re.compile(r'(?i)(?:Sex|Gender|الجنس)\s*[:\-]?\s*(Male|Female|M|F|ذكر|أنثى)'),
    re.compile(r'(?i)(?:Sex|Gender|الجنس)\s*[:\-]?\s*([^\n]{1,20})'),
]
ADDRESS_PATTERN = re.compile(r'(?i)(?:Address|العنوان)\s*[:\-]?\s*([^\n\r]{2,140})')
...
def parse_fields(text):
    ...
    # 1) Emirates ID number - use pre-compiled pattern
    m = EMIRATES_ID_PATTERN.search(raw)
    if m:
    ...
    name_patterns = NAME_PATTERNS  # Use pre-compiled patterns
