    ...
    if _paddle_ocr is None:
        try:
            ...
            _paddle_ocr = PaddleOCR(
                use_angle_cls=True,
                lang='en',
                use_gpu=True,  # Enable GPU if available (falls back to CPU automatically)
                ...
                det_db_unclip_ratio=1.5  # Optimize text region detection
            )
        except ImportError:
            raise ImportError(
        ...
        if is_pdf:
            ...
            try:
                ...
                for page_num in range(len(pdf_doc)):
                    ...
                    # Render page to image (pixmap) - optimized resolution
                    pix = page.get_pixmap(matrix=fitz.Matrix(1.5, 1.5))  # 1.5x zoom (balanced quality/speed)
...
def ocr_space_file_multi_lang(file_obj, is_pdf=False):
    ...
    try:
        ...
        # SUCCESS CRITERIA: Skip fallback if we have good quality text
        if code == 0 and len(text.strip()) > 100:  # Increased threshold from 40
    ...
    # Fallback to OCR.space ONLY if PaddleOCR didn't work well
    if not best_text or len(best_text.strip()) < 100:  # Increased threshold
        if is_pdf:
...
def process_pdf_page_by_page(file_obj):
    ...
    try:
        ...
        max_pages_to_process = min(total_pages, 5) # Rarely more than 5 pages needed
        ...
        def process_single_page(page_num):
            try:
                page = pdf_doc[page_num]
                # Render page to image - optimized resolution
                ...
                # Check if this page contains Emirates ID content
        ...
        # Use 2-4 threads depending on CPU
        with ThreadPoolExecutor(max_workers=min(max_pages_to_process, 4)) as executor:
            future_to_page = {executor.submit(process_single_page, p): p for p in range(max_pages_to_process)}
            
            # Use a counter for early termination (though parallel helps more here)
            emirates_id_found_count = 0
            MAX_EMIRATES_ID_PAGES = 2
            
            page_data_list = [None] * max_pages_to_process
            ...
            for future in as_completed(future_to_page):
                ...
                page_data_list[page_idx] = page_data
                
                if page_data['is_emirates_id']:
                    emirates_id_found_count += 1
                
                # If we've found enough Emirates ID pages, we could technically cancel others
                # But since they are likely running, we'll just wait or return early if possible
                # In threads, we can't easily cancel. But we can stop starting new ones if it were sequential.
            
        # Filter out None values and return
        final_list = [p for p in page_data_list if p is not None]
        pdf_doc.close()
        return final_list
        ...
        logger.error(f"Error in process_pdf_page_by_page: {e}")
        return []
